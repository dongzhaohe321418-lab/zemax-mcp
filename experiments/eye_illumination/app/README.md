...
这是 `eye-illumination-fixed-focal-60-120d-v3` 的本地交互应用。程序直接调用上级目录的 `eye_model.py` 和版本化参数，不会在前端复制或重新实现光学公式。界面采用紧凑的专业实验台布局；右上角的“中文 / EN”切换会同步更新静态文案、动态计算结果、图表、Zemax 状态和对应语言的 PDF 链接。应用包含两个明确分开的模式：
...
- 直接打开版本化中英文 PDF 报告、验证 JSON 和基准矩阵；“打开完整 PDF 报告”始终跟随当前界面语言。
