...
\date{2026年8月14日\quad 报告版本 1.0.0}
...
\begin{itemize}[leftmargin=2.2em]
  \item 在无外置镜片的 $100\mm$（$10\D$）工况，成人与儿童覆盖直径 $6\mm$ 后极部需要约 \keyresult{\AdultTenDDiameter\,mm} 的圆形光源；小鸡覆盖直径 $3\mm$ 后极部需要约 \keyresult{\ChickTenDDiameter\,mm}。
  ...
  \item 成人在 $100\mm$ 物距配合 $-10\D$ 外置镜片时需要 $17.004\D$ 调节，仍低于给定的 $18\D$ 上限；$-15\D$ 与 $-20\D$ 分别需要 $20.028\D$ 与 $22.787\D$，判为不可调节。
...
\Cref{tab:headline} 显示，光源靠近时实体直径显著减小，但眼内所需调节增大。小鸡与成人在 $20\D$ 工况分别越过 $15.69\D$ 与 $18\D$ 的调节上限；儿童在源文件给定的 $22.5\D$ 上限内仍可行。这里的“可行”仅表示调节力满足约束，尚未包含制造公差、像差与辐射安全约束。
...
\Cref{fig:source-diameter} 应按有序离散工况阅读：$2\D$ 到 $20\D$ 对应 $500\mm$ 到 $50\mm$ 物距。三条曲线高度接近的原因不是三个眼完全相同，而是目标后极部直径与有效焦距近似按比例变化，使所需角覆盖接近。设计含义是：先以约 $20\degree$ 完整角作为视场约束，再根据安装距离换算实体光源尺寸。
...
物方扫描 & $0,2,5,10,15,20\D$；有限物距 $s=1/|L|$ & 改变工作距离和调节需求 \\
外置镜片 & $0,-1,-3,-5,-10,-15,-20\D$ & 测试负镜片补偿与调节上限 \\
...
\end{equation}
数值质量控制要求所有扫描行满足 $|B_m|<10^{-12}\,\mathrm{m}$；完整的 105 行有限物距扫描全部通过。
...
\resizebox{0.97\textwidth}{!}{\begin{tikzpicture}[
  ...
  \node[check,below=of ppt] (tests) {21 项自动测试\\解析恒等式与边界};
...
\section{有限物距与外置镜片采用完整笛卡尔扫描}
三个眼模型分别扫描 7 个外置镜片水平和 5 个有限物距需求，共 $3\times7\times5=105$ 行聚焦结果；无穷远另有 $3\times7=21$ 行角覆盖结果。每行记录调节力、总眼屈光力、放大率、光源直径、光源面积、调节可行性与成像矩阵残差。
...
\section{三种眼模型在 10 D 工况给出相近的实体尺寸}
在 $100\mm$ 物距，无外置镜片时，成人和儿童需要 \AdultTenDDiameter\,mm 光源直径，小鸡需要 \ChickTenDDiameter\,mm。差异小于 $0.7\%$，说明第一阶段可以共享近似视场规格，但不意味着可以共享瞳孔、调节或机械顶点距设计。

\section{外置负镜片提高调节需求并略微增大光源尺寸}
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{adult_accommodation_cn.png}
\caption{成人模型在不同物方需求与外置负镜片度数下的调节需求。每个单元格为所需调节力（D）。}
\label{fig:adult-heatmap}
\end{figure}

\Cref{fig:adult-heatmap} 表明负镜片与近距目标共同推高调节需求；颜色只编码数值大小，单元格文本给出准确值。成人 $18\D$ 上限并未直接绘成二值颜色，以免把连续变化误读为只有“通过/失败”两种状态。
...
\begin{enumerate}[leftmargin=2.5em]
  ...
  \item \textbf{矩阵残差检查：}105 行有限物距结果的成像矩阵 $B_m$ 残差全部低于 $10^{-12}\,\mathrm{m}$。
...
\end{lstlisting}
该命令依次重建独立模型、CSV/JSON、五张基础图、C\# ZOS-API 验证器、六个 Zemax 系统、验证报告、Notebook、21 项测试和 HTML 报告。
...
\sourcefile{results/} & 105 行聚焦、96 行离焦、36 行眼轴、21 行无穷远及验证结果 \\
...
{\footnotesize\nolinkurl{experiments/runs/eye-illumination-650nm-v1.json}} & 不可覆盖的实验里程碑记录 \\
\bottomrule
\end{longtable}
...
在当前参数和近轴等效眼假设下，三个模型覆盖指定后极部的完整角需求均约为 $20\degree$；成人与儿童在 $100\mm$ 物距需要约 $35.93\mm$ 圆形光源，小鸡需要约 $35.71\mm$。负外置镜片会同时增加成人调节需求和光源尺寸，$-15\D$ 及更负的镜片在该工况越过成人调节上限。瞳孔主要影响离焦模糊而非理想聚焦像尺寸；保守离焦设计可维持覆盖，但会降低目标区域捕获效率。
...
\begin{itemize}[leftmargin=2.2em]
  ...
  \item 重新运行 21 项 Python 测试，检查 Git 差异和敏感信息，再提交并推送 GitHub。
\end{itemize}
