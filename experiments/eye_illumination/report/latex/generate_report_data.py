...
def main() -> None:
    ...
    for eye_id in eye_order:
        ...
        focal_rows_en.append(
            f"{eye_en[eye_id]} & {axial:.2f} & {target:.1f} & {pupil:.1f} & "
            + ", ".join(f"{f:g}" for f in focals) + " \\\\"
        )
        for focal in focals:
            ...
            endpoint_rows_en.append(
                f"{eye_en[eye_id]} & {focal:g} & {pupil:.1f} & "
                f"{num(row60['geometric_min_source_diameter_mm'])} & {num(row60['conservative_source_diameter_mm'])} & "
                f"{num(row120['geometric_min_source_diameter_mm'])} & {num(row120['conservative_source_diameter_mm'])} " + "\\\\"
            )
    ...
    def content_for(rows: list[str], endpoints: list[str]) -> list[str]:
        return [
    ...
    content = content_for(focal_rows, endpoint_rows)
    content_en = content_for(focal_rows_en, endpoint_rows_en)
    (HERE / "generated_results.tex").write_text("\n".join(content) + "\n", encoding="utf-8")
    (HERE / "generated_results_en.tex").write_text("\n".join(content_en) + "\n", encoding="utf-8")
    print(HERE / "generated_results.tex")
    print(HERE / "generated_results_en.tex")
